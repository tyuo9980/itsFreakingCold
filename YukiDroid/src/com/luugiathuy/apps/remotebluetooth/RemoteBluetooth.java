...
public class RemoteBluetooth extends Activity implements SensorEventListener
{
	...
	public void onSensorChanged(SensorEvent event)
	{
		...
		float degreey = Math.round(event.values[1]);
		...
		if (currentDegree > 50 && reset)
		{
			...
			if (state == 1)
			{
				mCommandService.write(BluetoothCommandService.LAND_L);
			}
		...
		} else if (currentDegree < -50 && reset)
		{
			...
			if (state == 1)
			{
				mCommandService.write(BluetoothCommandService.LAND_R);
			}
		}
		else if (degreey > -30 && reset)
		{
			...
			if (state == 1)
			{
				mCommandService.write(BluetoothCommandService.THROW);
			}
		}
		else if (currentDegree < 30 && currentDegree > -30 && degreey < -60 && degreey > -120)
		{
			reset = true;
		}
